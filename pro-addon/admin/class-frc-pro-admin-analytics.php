...
class FRC_Pro_Admin_Analytics {
	...
	public function render() {
		...
		<div class="wrap frc-wrap">
			<h1><?php esc_html_e( 'Recovery Analytics', 'flexi-revive-cart-pro' ); ?></h1>
			...
			<nav class="nav-tab-wrapper">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=frc-analytics&tab=recovery' ) ); ?>" class="nav-tab <?php echo 'recovery' === $active_tab ? 'nav-tab-active' : ''; ?>">
					<?php esc_html_e( 'Channel Analytics', 'flexi-revive-cart-pro' ); ?>
				</a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=frc-analytics&tab=whatsapp' ) ); ?>" class="nav-tab <?php echo 'whatsapp' === $active_tab ? 'nav-tab-active' : ''; ?>">
					<?php esc_html_e( 'WhatsApp Campaigns', 'flexi-revive-cart-pro' ); ?>
				</a>
			</nav>
	...
	/**
	 * Render channel-level recovery analytics.
	 */
	private function render_channel_analytics() {
		...
		?>
		<h2><?php esc_html_e( 'Recovery Analytics by Channel', 'flexi-revive-cart-pro' ); ?></h2>

		<form method="get" style="margin-bottom:16px;">
			<input type="hidden" name="page" value="frc-analytics" />
			<input type="hidden" name="tab" value="recovery" />
			<label>
				<?php esc_html_e( 'From:', 'flexi-revive-cart-pro' ); ?>
				<input type="date" name="date_from" value="<?php echo esc_attr( $date_from ); ?>" />
			</label>&nbsp;
			<label>
				<?php esc_html_e( 'To:', 'flexi-revive-cart-pro' ); ?>
				<input type="date" name="date_to" value="<?php echo esc_attr( $date_to ); ?>" />
			</label>&nbsp;
			<label>
				<?php esc_html_e( 'Channel:', 'flexi-revive-cart-pro' ); ?>
				<select name="channel">
					<option value=""><?php esc_html_e( 'All', 'flexi-revive-cart-pro' ); ?></option>
					<option value="email" <?php selected( $filter_channel, 'email' ); ?>><?php esc_html_e( 'Email', 'flexi-revive-cart-pro' ); ?></option>
					<option value="sms" <?php selected( $filter_channel, 'sms' ); ?>><?php esc_html_e( 'SMS', 'flexi-revive-cart-pro' ); ?></option>
					<option value="whatsapp" <?php selected( $filter_channel, 'whatsapp' ); ?>><?php esc_html_e( 'WhatsApp', 'flexi-revive-cart-pro' ); ?></option>
					<option value="push" <?php selected( $filter_channel, 'push' ); ?>><?php esc_html_e( 'Push', 'flexi-revive-cart-pro' ); ?></option>
				</select>
			</label>&nbsp;
			<button type="submit" class="button"><?php esc_html_e( 'Filter', 'flexi-revive-cart-pro' ); ?></button>
		</form>
		<?php
		...
		foreach ( $channels as $channel ) {
			...
			$stats = $wpdb->get_row( $log_query ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			...
			$rec_query  = $wpdb->prepare(
				...
				WHERE c.recovery_channel = %s AND c.status IN ('recovered','converted')" . $carts_date_where, // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
				$rec_params
			);
			$rec = $wpdb->get_row( $rec_query ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared

			$sent = (int) $stats->sent;
			$rate = $sent > 0 ? round( ( (int) $rec->recovered / $sent ) * 100, 1 ) : 0;
		...
		echo '<thead><tr>';
		echo '<th>' . esc_html__( 'Channel', 'flexi-revive-cart-pro' ) . '</th>';
		echo '<th>' . esc_html__( 'Sent', 'flexi-revive-cart-pro' ) . '</th>';
		echo '<th>' . esc_html__( 'Opened', 'flexi-revive-cart-pro' ) . '</th>';
		echo '<th>' . esc_html__( 'Clicked', 'flexi-revive-cart-pro' ) . '</th>';
		echo '<th>' . esc_html__( 'Recovered', 'flexi-revive-cart-pro' ) . '</th>';
		echo '<th>' . esc_html__( 'Revenue', 'flexi-revive-cart-pro' ) . '</th>';
		echo '<th>' . esc_html__( 'Rate', 'flexi-revive-cart-pro' ) . '</th>';
		echo '</tr></thead><tbody>';
	...
	/**
	 * Render WhatsApp campaigns table.
	 */
	private function render_whatsapp_campaigns() {
		...
		echo '<h2>' . esc_html__( 'WhatsApp Bulk Campaigns', 'flexi-revive-cart-pro' ) . '</h2>';

		if ( empty( $campaigns ) ) {
			echo '<p>' . esc_html__( 'No campaigns sent yet.', 'flexi-revive-cart-pro' ) . '</p>';
			return;
		}
