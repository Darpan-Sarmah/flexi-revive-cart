...
class FRC_REST_API {
	 ...
	 * @var string
	 */
	const NAMESPACE = 'flexi-revive-cart/v1';
		...
		register_rest_route(
			...
			array(
				...
				'args'                => array(
					'status'     => array(
						'type'    => 'string',
						'enum'    => array( 'abandoned', 'recovered', 'converted', 'expired' ),
					),
	...
	public function check_permission() {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			return new WP_Error( 'rest_forbidden', __( 'You do not have permission to access this endpoint.', 'flexi-revive-cart' ), array( 'status' => 403 ) );
		}
		return true;
	}
	...
	public function get_cart( $request ) {
		...
		if ( ! $cart ) {
			return new WP_Error( 'not_found', __( 'Cart not found.', 'flexi-revive-cart' ), array( 'status' => 404 ) );
		}
	...
	public function trigger_recovery( $request ) {
		...
		return rest_ensure_response( array(
			'success' => $sent,
			'message' => $sent ? __( 'Reminder sent.', 'flexi-revive-cart' ) : __( 'Failed to send reminder.', 'flexi-revive-cart' ),
		) );
	}
