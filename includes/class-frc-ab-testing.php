...
/**
 * Class FRC_AB_Testing
 */
class FRC_AB_Testing {

	/**
	 * Get the subject line to use for a cart, applying A/B variant if applicable.
	 *
	 * @param int    $cart_id       Cart ID (used for consistent variant assignment).
	 * @param string $default_subject Default subject line.
	 * @param int    $stage         Email stage.
	 * @return string
	 */
	public function get_subject_for_cart( $cart_id, $default_subject, $stage ) {
		if ( ! FRC_PRO_ACTIVE ) {
		...
		$variant  = $this->assign_variant( $cart_id );
		$subject  = 'a' === $variant ? $active_test->variant_a : $active_test->variant_b;
	...
	/**
	 * Assign a variant (a/b) deterministically based on cart ID.
	 *
	...
	/**
	 * Fetch the active A/B test for a given type and stage.
	 *
